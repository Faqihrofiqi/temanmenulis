...
<body class="bg-slate-50 text-slate-900 antialiased font-['Space_Grotesk'] transition-colors duration-300 dark:bg-slate-950 dark:text-slate-100">
    <div class="relative min-h-screen overflow-hidden">
        ...
        <header class="sticky top-0 z-40 border-b border-slate-200/80 bg-white/80 backdrop-blur dark:border-white/5 dark:bg-slate-950/70">
            <div class="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
                ...
                <div class="flex items-center gap-3 text-sm">
                    ...
                    @auth
                        <a href="{{ route('dashboard') }}" class="rounded-full border border-slate-300/80 px-4 py-2 font-semibold text-slate-900 transition hover:border-slate-500 dark:border-white/20 dark:text-white dark:hover:border-white/60">Dashboard</a>
                    @else
                        <a href="{{ route('login') }}" class="rounded-full border border-slate-300/80 px-4 py-2 text-slate-900 transition hover:border-slate-500 dark:border-white/20 dark:text-white dark:hover:border-white/60">Masuk</a>
                        @if (Route::has('register'))
                            <a href="{{ route('register') }}" class="rounded-full bg-slate-900 px-4 py-2 font-semibold text-white transition hover:bg-slate-800 dark:bg-white dark:text-slate-900 dark:hover:bg-slate-100">Daftar</a>
                        @endif
                    @endauth
                </div>
            </div>
